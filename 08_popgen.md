...
* Using filtered biallelic SNPS from 03...

* Convert vcf to plink format. use newest version of plink2 and rename chromosomes scaffold_01 > 1, e.g

```
plink2 --vcf biallelic_SNPS.vcf --make-pgen --out sorted_pgen_fileset --sort-vars --rename-chrs rename.txt  --chr-set 29
plink2 --pfile sorted_pgen_fileset --make-bed --out biallelic_SNPs_sorted
```

*get estimates of structure from admixture

```
for i in {2..5}; do admixture --cv biallelic_SNPs_sorted.bed ${i} > log${i}.out; done
...
* Choose 2, as it is consistent with phylogeny. One outlier possibly due to contamination based on odd result from mat gene screening suggesting likley contamination.

# calculate linkage disequilibrium. Look among populations. 

e.g. for coffee rust. rename chromosomes using new plink2 function. use rename key rename.txt and set the number of chromosomes. remove variants mapped to unplaced contigs
```
...
plink2 -pfile sorted_pgen_fileset --ld-window 100 --ld-window-kb 1000 --r2-unphased --ld-window-r2 0
```
